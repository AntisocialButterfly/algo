...
public class Main {
  ...
  private static class CheckChecker {
    public static enum CheckResult {
      WHITE("white king"), BLACK("black king"), NONE("no king");
    ...
    private static Map<Character, int[][]> attackVectors = new HashMap<Character, int[][]>();

    static {
      int[][] whitePawnAttackVectors = new int[][] { { getVectorTo(-1, -1) },
          { getVectorTo(-1, 1) } };

      int[][] blackPawnAttackVectors = new int[][] { { getVectorTo(1, 1) }, { getVectorTo(1, -1) } };

      int[][] rookAttackVectors = new int[][] { repeat(getVectorTo(1, 0)),
          repeat(getVectorTo(-1, 0)), repeat(getVectorTo(0, 1)), repeat(getVectorTo(0, -1)), };

      int[][] bishopAttackVectors = new int[][] { repeat(getVectorTo(1, 1)),
          repeat(getVectorTo(-1, 1)), repeat(getVectorTo(-1, -1)), repeat(getVectorTo(1, -1)), };

      int[][] queenAttackVectors = new int[][] { repeat(getVectorTo(1, 0)),
          repeat(getVectorTo(1, 1)), repeat(getVectorTo(0, 1)), repeat(getVectorTo(-1, 1)),
          repeat(getVectorTo(-1, 0)), repeat(getVectorTo(-1, -1)), repeat(getVectorTo(0, -1)),
          repeat(getVectorTo(1, -1)), };

      int[][] knightAttackVectors = new int[][] { { getVectorTo(2, 1) }, { getVectorTo(1, 2) },
          { getVectorTo(-1, 2) }, { getVectorTo(-2, 1) }, { getVectorTo(-2, -1) },
          { getVectorTo(-1, -2) }, { getVectorTo(1, -2) }, { getVectorTo(2, -1) }, };

      int[][] kingAttackVectors = new int[][] { { getVectorTo(1, 0) }, { getVectorTo(1, 1) },
          { getVectorTo(0, 1) }, { getVectorTo(-1, 1) }, { getVectorTo(-1, 0) },
          { getVectorTo(-1, -1) }, { getVectorTo(0, -1) }, { getVectorTo(1, -1) }, };
    ...
    public static CheckChecker create(BufferedReader reader) throws IOException,
    ...
    private CheckResult checkAttacks(int attackerPosition, char attackingPiece) {
  ...
  public static void main(String[] args) throws IOException {
    ...
    try {
      do {
        System.out.printf("Game #%d: %s is in check.%n", n, CheckChecker.create(reader).check());
        reader.readLine();
        n++;
      } while (true);
